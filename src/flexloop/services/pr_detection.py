...
async def check_prs(
    ...
    if not current_1rm_pr or estimated_1rm > current_1rm_pr.value:
        ...
        new_prs.append({
            ...
            "detail": f"{round(weight, 1)}{weight_unit} x {reps} reps",
        })
    ...
    if current_rep_pr:
        ...
        if reps > current_rep_pr.value:
            ...
            new_prs.append({
                ...
                "detail": f"{reps} reps at {round(weight, 1)}{weight_unit}",
            })
    else:
        pr = PersonalRecord(
            user_id=user_id,
    ...
    if current_volume_pr:
        if volume > current_volume_pr.value:
            ...
            new_prs.append({
                ...
                "detail": f"{round(weight, 1)}{weight_unit} x {reps} = {round(volume, 1)}{weight_unit} volume",
            })
    else:
        pr = PersonalRecord(
            user_id=user_id,
